...
const ActivityFeed = ({ walletAddress = '0xabcd...ef01' }) => {
  const {
    activities,
    statistics,
    loading,
  ...
  const ActivityItem = ({ activity }: { activity: typeof activities[number] }) => (
    <div className="bg-background rounded-lg border border-border hover:border-gray-300 dark:hover:border-gray-600 transition-all duration-200 hover:scale-105 p-6">
            ...
            <div className="space-y-1 text-sm text-gray-600 dark:text-gray-300">
              {activity.formattedPrice && (
                <div className="flex items-center">
                  ...
                  <span className="font-medium">{activity.formattedPrice}</span>
                </div>
              )}
            ...
            {activity.metadata && Object.keys(activity.metadata).length > 0 && (
              <div className="mt-3 flex flex-wrap gap-2">
                {Object.entries(activity.metadata).map(([key, value]) => (
                  <span key={key} className="inline-flex items-center px-2 py-1 rounded text-xs bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300">
  ...
  return (
    <div className="container mx-auto px-4 py-8 mt-5 mb-10">
      ...
      <div className="mb-8 mx-6">
        <h1 className="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-2">Activity Feed</h1>
        <p className="text-gray-600 dark:text-gray-300">Track your onchain activities and transactions</p>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg border transition-colors flex items-center space-x-2 ${
              showFilters 
                ...
                : 'bg-background border-border text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-800'
            }`}
          >
      ...
      {/* Statistics */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 mx-6">
        ...
        {loading ? (
          ...
          <div className="text-center py-12 bg-gray-50 dark:bg-gray-800/50 rounded-lg">
            ...
            <p className="text-gray-600 dark:text-gray-300 mb-4">
              {searchTerm ? 'Try adjusting your search terms' : 'Start by minting your first IP or making an offer'}
            </p>
            {searchTerm && (
              <button
      ...
      {filteredActivities.length > 0 && pagination.hasMore && (
        <div className="text-center mt-8 mx-6">
          <button 
            onClick={loadMore}
            disabled={loading}
