...
export function useCollection(): UseCollectionReturn {
  ...
  const uploadMetadataToIPFS = useCallback(async (metadata: CollectionMetadata): Promise<string> => {
    setIsUploading(true);
    setError(null);
  ...
  const createCollection = useCallback(async (formData: CollectionFormData) => {
    if (!address) {
    ...
    try {
      ...
      const symbol = formData.name.replace(/[^a-zA-Z0-9]/g, '').substring(0, 6).toUpperCase() || 'COLL';
