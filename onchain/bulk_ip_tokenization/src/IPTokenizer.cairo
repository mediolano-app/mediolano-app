...
mod IPTokenizer {
    ...
    use core::{
        ...
        option::OptionTrait,
        starknet::{
            ContractAddress, get_caller_address, storage::{
    ...
    // Interface import
    use super::super::interfaces::{IBulkIPTokenizer, IIPNFTDispatcher, IIPNFTDispatcherTrait};
    // Type imports
    ...
    impl Private of PrivateTrait {
        fn _mint(ref self: ContractState, asset: IPAssetData) -> u256 {
            ...
            let ipnft_dispatcher = IIPNFTDispatcher { contract_address: self.nft_contract.read()};
            let token_id = ipnft_dispatcher.mint(asset.clone().owner);
            self.token_counter.write(token_id);
            self.token_owner.write(token_id, asset.clone().owner);
